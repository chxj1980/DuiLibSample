...
{
	return std::pow(state, 2);
}

void GameListUI::Node::set_parent(Node * parent)
{
	_parent = parent;
}

GameListUI::Node::Node()
{
	_parent = nullptr;
}

GameListUI::Node::Node(NodeData t)
{
	_data = t;
	_parent = nullptr;
}

GameListUI::Node::Node(NodeData t, Node * parent)
{
	_data = t;
	_parent = parent;
}
...
GameListUI::NodeData & GameListUI::Node::data()
{
	// TODO: 在此处插入 return 语句
	return _data;
}

int GameListUI::Node::num_children() const
{
	return _children.size();
}

GameListUI::Node * GameListUI::Node::child(int i)
{
	return _children[i];
}

GameListUI::Node * GameListUI::Node::parent()
{
	return _parent;
}

bool GameListUI::Node::has_children() const
{
	return num_children() > 0;
}

void GameListUI::Node::add_child(Node * child)
{
	child->set_parent(this);
	_children.push_back(child);
}

void GameListUI::Node::remove_child(Node * child)
{
...
GameListUI::Node * GameListUI::Node::get_last_child()
{
	if (has_children())
	{
...
bool GameListUI::Add(DuiLib::CControlUI * pControl)
{
...
bool GameListUI::AddAt(CControlUI * pControl, int iIndex)
{
	if (!pControl) return false;
	if (_tcscmp(pControl->GetClass(), _T("ListLabelElementUI")) != 0) return false;
...
bool GameListUI::Remove(CControlUI * pControl)
{
	if (!pControl) return false;
	if (_tcscmp(pControl->GetClass(), _T("ListLabelElementUI")) != 0) return false;

	if (reinterpret_cast<Node*>(static_cast<DuiLib::CListLabelElementUI*>(pControl->GetInterface(_T("ListLabelElement")))->GetTag()) == NULL)
		return CListUI::Remove(pControl);
	else
		return RemoveNode(reinterpret_cast<Node*>(static_cast<DuiLib::CListLabelElementUI*>(pControl->GetInterface(_T("ListLabelElement")))->GetTag()));
}

bool GameListUI::RemoveAt(int iIndex)
{
...
void GameListUI::SetVisible(bool bVisible)
{
	if (m_bVisible == bVisible) return;
	CControlUI::SetVisible(bVisible);
}

void GameListUI::SetInternVisible(bool bVisible)
{
	CControlUI::SetInternVisible(bVisible);
}

void GameListUI::DoEvent(DuiLib::TEventUI & event)
{
...
GameListUI::Node * GameListUI::GetRoot()
{
	return _root;
}

GameListUI::Node * GameListUI::AddNode(LPCTSTR text, Node * parent)
{
...
bool GameListUI::RemoveNode(Node * node)
{
...
void GameListUI::ExpandNode(Node * node, bool expand)
{
...
SIZE GameListUI::GetExpanderSizeX(Node * node) const
{
	if (!node || node == _root) return DuiLib::CDuiSize();
	if (node->data()._level >= 3) return DuiLib::CDuiSize();
