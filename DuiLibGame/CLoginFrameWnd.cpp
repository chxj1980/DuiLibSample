...
{
	DuiLib::CComboUI* pAccountCombo = static_cast<DuiLib::CComboUI*>(m_pm.FindControl(_T("accountcombo")));
	DuiLib::CEditUI* pAccountEdit = static_cast<DuiLib::CEditUI*>(m_pm.FindControl(_T("accountedit")));
	if (pAccountCombo && pAccountEdit) pAccountEdit->SetText(pAccountCombo->GetText());
	pAccountEdit->SetFocus();
}

void CLoginFrameWnd::Notify(DuiLib::TNotifyUI & msg)
{
...
LRESULT CLoginFrameWnd::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL & bHandled)
{
	...
	DuiLib::CControlUI* pRoot = builder.Create(_T("login.xml"), (UINT)0, &cb, &m_pm);
