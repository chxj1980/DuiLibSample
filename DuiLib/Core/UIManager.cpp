...
namespace DuiLib {
	...
	CDuiString CPaintManagerUI::m_pStrDefaultFontName;    //added by cddjr at 05/18/2012，调用默认构造函数，字符串容量为63+1
	...
	short CPaintManagerUI::m_H = 180;					  //色调
	short CPaintManagerUI::m_S = 100;					  //饱和度
	short CPaintManagerUI::m_L = 100;					  //亮度
	CStdPtrArray CPaintManagerUI::m_aPreMessages;         //容器，存储的元素类型为 CPaintManagerUI 这个容器把自己的引用也装进去了
	CStdPtrArray CPaintManagerUI::m_aPlugins;			  //插件容器，初始容量为0
	...
	{
		m_dwDefaultDisabledColor = 0xFFA7A6AA;     //argb
		...
		LOGFONT lf = { 0 };
		::GetObject(::GetStockObject(DEFAULT_GUI_FONT), sizeof(LOGFONT), &lf);//获取默认GUI字体
		lf.lfCharSet = DEFAULT_CHARSET;            //设置默认字符集
		if (CPaintManagerUI::m_pStrDefaultFontName.GetLength() > 0)           //获取到默认字体名，则存储在变量 CPaintManagerUI::m_pStrDefaultFontName 中
		{
	...
	void CPaintManagerUI::Init(HWND hWnd)
	{
		...
		m_aPreMessages.Add(this);  //容器中缓存对自身的引用，思考下这样做的目的
	}
	...
	bool CPaintManagerUI::LoadPlugin(LPCTSTR pstrModuleName)
	{
		...
		if (hModule != NULL) {
			LPCREATECONTROL lpCreateControl = (LPCREATECONTROL)::GetProcAddress(hModule, "CreateControl");//需要理解  ?
			if (lpCreateControl != NULL) {
				if (m_aPlugins.Find(lpCreateControl) >= 0) return true;//已经加载过，则不用再次加载
				m_aPlugins.Add(lpCreateControl);					   //没有加载过，则纳入缓存管理
				return true;
			}
		}
		return false;
	...
	//调用 TranslateAccelerator 为了分发消息？
