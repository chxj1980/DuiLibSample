...
//
	...
	public:
		...
		static bool LoadPlugin(LPCTSTR pstrModuleName);//加载插件，存入 m_aPlugins 中
		...
		bool AddTranslateAccelerator(ITranslateAccelerator* pTranslateAccelerator);   //添加 TranslateAccelerator
		bool RemoveTranslateAccelerator(ITranslateAccelerator* pTranslateAccelerator);//删除 TranslateAccelerator
		bool TranslateAccelerator(LPMSG pMsg);										  //调用 TranslateAccelerator 为了分发消息？
	...
	private:
		...
		static short m_H;						//色调
		static short m_S;						//饱和度
		static short m_L;						//亮度
		static CStdPtrArray m_aPreMessages;     //CPaintManagerUI 类型数组， 当调用Init()方法时，这个容器还会将自己的引用缓存起来
		static CStdPtrArray m_aPlugins;			//插件数组，存储一系列插件的容器

	public:
		static CDuiString m_pStrDefaultFontName;//这个变量好像没什么用，只是在构造CPaintManager时，获取了一下系统默认的字体名称，然后存储在这个变量中
		CStdPtrArray m_aTranslateAccelerator;   //消息分发器数组？
	};
