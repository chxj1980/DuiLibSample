...
#define MSGDEF(x) if(uMsg==x) return _T(#x)
	...
	LRESULT CALLBACK CWindowWnd::__WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		if (uMsg == WM_NCCREATE) {
			...
			pThis->m_hWnd = hWnd;                                                    //CWindowWnd保持窗口的句柄 --》产生关联
			::SetWindowLongPtr(hWnd, GWLP_USERDATA, reinterpret_cast<LPARAM>(pThis));//窗口保持CWindowWnd引用   --》产生关联
		}
		else {
			pThis = reinterpret_cast<CWindowWnd*>(::GetWindowLongPtr(hWnd, GWLP_USERDATA));
			if (uMsg == WM_NCDESTROY && pThis != NULL) {
				LRESULT lRes = ::CallWindowProc(pThis->m_OldWndProc, hWnd, uMsg, wParam, lParam);//将要销毁时，获取到的消息默认处理就好
				::SetWindowLongPtr(pThis->m_hWnd, GWLP_USERDATA, 0L);                            //解除窗口对CWindowWnd的引用    --》解除关联
				if (pThis->m_bSubclassed) pThis->Unsubclass();                                   //超类化窗体
				pThis->m_hWnd = NULL;                                                            //解除CWindowWnd对窗口句柄的引用 --》解除关联
				pThis->OnFinalMessage(hWnd);                                                     //最后一步处理~~~
				return lRes;
			}
		}
		if (pThis != NULL) {
			return pThis->HandleMessage(uMsg, wParam, lParam);                       //一般的业务逻辑相关的消息，分派给子类处理
		}
		else {
